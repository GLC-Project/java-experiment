 ...
 * 
 * @author Dmitry Lavrov - dmtrl - OpenAHP - Omsk Java User Group - GPL 3.0
 */
class BigDecimalMath {
     ...
     * \protect\vrule
     * width0pt\protect\href{http://www.cs.arizona.edu/icon/oddsends/e.htm}{http
     * ://www.cs.arizona.edu/icon/oddsends/e.htm} The precision of the
    ...
    /**
     * Euler’s constant Pi. \protect\vrule
     * width0pt\protect\href{http://www.cs.arizona.edu/icon/oddsends/pi.htm}{
    ...
    /**
     * Euler-Mascheroni constant lower-case gamma. 13 \protect\vrule
     * width0pt\protect\href{http://www.worldwideschool.org/library/books/sci/
     * math/MiscellaneousMathematicalConstants/chap35.html}{http:/
     */
    ...
    /**
     * Natural logarithm of 2. \protect\vrule
     * width0pt\protect\href{http://www.worldwideschool.org/library/books/sci/
     * math/MiscellaneousMathematicalConstants/chap58.html}{http:/
     */
	...
	if (mc.getPrecision() < PI.precision()) {
	     ...
	     * width0pt\protect\href{http://arxiv.org/abs/math/9803067}{arXiv:
	     * math/9803067}
	     */
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
	    throw new ArithmeticException(
		    "negative argument " + x.toString() + " of square root");
	}
	return root(2, x);
    } /* BigDecimalMath.sqrt */
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
	    throw new ArithmeticException(
		    "negative argument " + x.toString() + " of root");
	}
	if (n <= 0) {
	 ...
	 */
	BigDecimal zerr = x.abs().multiply(x.ulp())
		.add(y.abs().multiply(y.ulp()));
	 ...
	 */
	mc = new MathContext(err2prec(z.doubleValue(),
		0.5 * zerr.doubleValue() / z.doubleValue()));
	return z.round(mc);
	 ...
	 */
	mc = new MathContext(
		err2prec(z.doubleValue(), 0.5 * zerr / z.doubleValue()));
	return z.round(mc);
    } /* BigDecimalMath.hypot */
     ...
     *         precision in the argument. 16 In particular this means that
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
	    ...
	    return scalePrec(BigDecimal.ONE,
		    -(int) (Math.log10(x.ulp().doubleValue())));
	} else {
	    /*
		 ...
		 */
		MathContext mcTay = new MathContext(
			err2prec(1., xUlpDbl / TAYLOR_NTERM));
		...
		int exSc = (int) (1.0 - Math
			.log10(TAYLOR_NTERM * (TAYLOR_NTERM - 1.0)
		 ...
		 * 8 to avoid an invalid operation 17 response by the
		 * BigDecimal.pow library or integer overflow.
		 */
		while (exSc > 0) {
		    int exsub = Math.min(8, exSc);
		    exSc -= exsub;
		    MathContext mctmp = new MathContext(
			    expxby10.precision() - exsub + 2);
		    int pex = 1;
		    while (exsub-- > 0) {
			pex *= 10;
		    }
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
	    throw new ArithmeticException(
		    "Cannot take log of negative " + x.toString());
	    ...
	    MathContext mc = new MathContext(
		    err2prec(resul.doubleValue(), eps));
	    return resul.round(mc);
	} else {
	    final double xDbl = x.doubleValue();
	    final double xUlpDbl = x.ulp().doubleValue();
	     ...
	     */
	    MathContext mc = new MathContext(
		    err2prec(resul.doubleValue(), xUlpDbl / xDbl));
	    return resul.round(mc);
	}
    } /* BigDecimalMath.log */
	...
	if (n <= 0) {
		 ...
		 * arXiv:math/9803067} Error propagation: the error in log(2) is
		 * twice the error in S(2,-5,...).
		 */
		int[] a = { 2, -5, -2, -7, -2, -5, 2, -3 };
		BigDecimal S = broadhurstBBP(2, 1, a,
			new MathContext(1 + mc.getPrecision()));
	...
	if (r.compareTo(Rational.ZERO) <= 0) {
	    throw new ArithmeticException(
		    "Cannot take log of negative " + r.toString());
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
	    throw new ArithmeticException(
		    "Cannot power negative " + x.toString());
	} else if (x.compareTo(BigDecimal.ZERO) == 0) {
	    return BigDecimal.ZERO;
	} else {
	    /*
	     ...
	     */
	    double errR = Math
		    .abs(logx.doubleValue() * y.ulp().doubleValue() / 2.)
	...
	/*
	 * The relative error in the result is n times the relative error in the
	 * input. The estimation is slightly optimistic due to the integer
	 * rounding of the logarithm.
	 */
	MathContext mc = new MathContext(
		x.precision() - (int) Math.log10((Math.abs(n))));
	return x.pow(n, mc);
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
		...
		if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
		     ...
		     */
		    return cos(
			    subtractRound(p.divide(new BigDecimal("2")), res));
		} else {
		    /*
		     ...
		     */
		    int k = (int) (res.precision()
			    / Math.log10(1.0 / res.doubleValue())) / 2;
		    MathContext mcTay = new MathContext(
			    err2prec(res.doubleValue(), xUlpDbl / k));
		    for (int i = 1;; i++) {
			/*
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
		...
		if (res.multiply(new BigDecimal("4")).compareTo(p) > 0) {
		     ...
		     */
		    int k = (int) (Math.log(xUlpDbl)
			    / Math.log(res.doubleValue())) / 2;
		    MathContext mcTay = new MathContext(
			    err2prec(1., xUlpDbl / k));
		    for (int i = 1;; i++) {
			/*
		     ...
		     */
		    mc = new MathContext(
			    err2prec(resul.doubleValue(), xUlpDbl));
		    return resul.round(mc);
		}
	    }
	}
	...
	if (x.compareTo(BigDecimal.ZERO) == 0) {
		...
		for (int i = 2;; i++) {
		    ...
		    f = f.multiply(fourn)
			    .multiply(fourn.subtract(BigInteger.ONE))
			    .divide(fac);
	...
	if (x.compareTo(BigDecimal.ZERO) == 0) {
	    throw new ArithmeticException(
		    "Cannot take cot of zero " + x.toString());
	} else if (x.compareTo(BigDecimal.ZERO) < 0) {
	    ...
	    final BigDecimal xhighprSq = multiplyRound(xhighpr, xhighpr);
	    MathContext mc = new MathContext(
		    err2prec(xhighpr.doubleValue(), eps));
	...
	if (x.compareTo(BigDecimal.ONE) > 0
	    ...
	    throw new ArithmeticException(
		    "Out of range argument " + x.toString() + " of asin");
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
		...
		int k = (int) (Math.log(xUlpDbl) / Math.log(x.doubleValue()))
			/ 2;
		/*
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
		...
		BigDecimal resul = sinh(xhalf).multiply(cosh(xhalf))
			.multiply(two);
		/*
		...
		MathContext mc = new MathContext(
			err2prec(0.5 * x.ulp().doubleValue() / eps));
		...
		int k = (int) (x.precision()
			/ Math.log10(1.0 / xhighpr.doubleValue())) / 2;
		MathContext mcTay = new MathContext(
			err2prec(x.doubleValue(), xUlpDbl / k));
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
	    ...
	    MathContext mc = new MathContext(
		    err2prec(Math.tanh(x.doubleValue()), eps));

	    return BigDecimal.ONE.subtract(exp2x)
		    .divide(BigDecimal.ONE.add(exp2x), mc);
	...
	if (x.compareTo(BigDecimal.ONE) < 0) {
	    throw new ArithmeticException(
		    "Out of range argument cosh " + x.toString());
	} else if (x.compareTo(BigDecimal.ONE) == 0) {
	    return BigDecimal.ZERO;
	} else {
	    BigDecimal xhighpr = scalePrec(x, 2);
	     ...
	     */
	    BigDecimal logx = log(
		    sqrt(xhighpr.pow(2).subtract(BigDecimal.ONE)).add(xhighpr));
	    /*
	...
	if (x.compareTo(BigDecimal.ZERO) < 0) {
		...
		for (int n = 2;; n++) {
		    ...
		    MathContext m = new MathContext(err2prec(
			    n * z.ulp().doubleValue() / 2. / z.doubleValue()));
		    c = c.round(m);
		    /*
			...
			m = new MathContext(
				err2prec(eps / 100. / c.doubleValue()));
		    } else {
			m = new MathContext(2);
		    }
		    /* zeta(n) -1 */
	...
	if (n < 0) {
	    throw new ProviderException(
		    "Unimplemented pochhammer with negative index " + n);
	} else if (n == 0) {
	    return BigDecimal.ONE;
	} else {
	    /*
	 ...
	 */
	mc = new MathContext(
		err2prec(res.doubleValue(), x.ulp().doubleValue() / 2.));
     ...
     * 
     * @param n
     *            The positive integer argument. 32
     * @param mc
	...
	if (n <= 0) {
	    throw new ProviderException(
		    "Unimplemented zeta at negative argument " + n);
	}
	...
	if (n % 2 == 0) {
	     ...
	     */
	    MathContext mcpi = new MathContext(
		    mc.getPrecision() + (int) (Math.log10(10.0 * n)));
	    ...
	    /*
	     * Broadhurst BBP \protect\vrule
	     * width0pt\protect\href{http://arxiv.org/abs/math/9803067}{arXiv:
	     * math/9803067} Error propagation: S31 is roughly 0.087, S33
	     * roughly 0.131
	     */
	     ...
	     * Broadhurst BBP \protect\vrule
	     * width0pt\protect\href{http://arxiv.org/abs/math/9803067}{arXiv:
	     * math/9803067} Error propagation: S51 is roughly -11.15, S53
	     * roughly 22.165, S55 is roughly 0.031 9*2048*S51/6265 = -3.28.
	     * 7*2038*S53/61651= 5.07. 738*2048*S55/61651= 0.747. The result is
	     * of the order 1.03, so we add 2 digits to S51 and S52 and one
	     * digit to S55.
	     */
	    ...
	    BigDecimal S51 = broadhurstBBP(5, 1, a51,
		    new MathContext(2 + mc.getPrecision()));
	    BigDecimal S53 = broadhurstBBP(5, 3, a53,
		    new MathContext(2 + mc.getPrecision()));
	    BigDecimal S55 = broadhurstBBP(5, 5, a55,
		    new MathContext(1 + mc.getPrecision()));
	    ...
	    MathContext mcloc = new MathContext(
		    2 + mc.getPrecision() + (int) (Math.log10((n))));
		 ...
		 */
		BigDecimal exp2p = pi(
			new MathContext(3 + err2prec(3.14, eps / 0.0075)));
		 ...
		 */
		BigDecimal twop = pi(
			new MathContext(3 + err2prec(3.14, eps / 0.017)));
     ...
     * @return S_(n,p)(a)
     * @see \protect\vrule
     *      width0pt\protect\href{http://arxiv.org/abs/math/9803067}{arXiv:math/
     *      9803067}
     */
	 ...
	 */
	MathContext mc = new MathContext(
		n.compareTo(BigInteger.ZERO) != 0 ? x.precision() : 0);
     ...
     */
    static public BigDecimal divideRound(final BigDecimal x,
	    final BigDecimal y) {
	/*
	 * The estimation of the relative error in the result is
	 * |err(y)/y|+|err(x)/x|
     ...
     */
    static public BigDecimal divideRound(final BigInteger n,
	    final BigDecimal x) {
	/*
	 * The estimation of the relative error in the result is |err(x)/x|
	 */
     ...
     *         precision.
     */
    static public BigDecimal scalePrec(final BigDecimal x,
