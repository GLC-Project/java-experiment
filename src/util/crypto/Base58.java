...
/**
 * <p>
 * Base58 is a way to encode Litecoin addresses as numbers and letters. Note
 * that this is not the same base58 as used by Flickr, which you may see
 * reference to around the internet.
 * </p>
 * 
 * <p>
...
public class Base58 {
	...
	for (int i = 0; i < input.length(); ++i) {
	    ...
	    if (digit58 < 0) {
		throw new Exception("Illegal character " + c + " at " + i); //$NON-NLS-1$ //$NON-NLS-2$
	    }
	...
	if (tmp.length < 4)
	    throw new Exception("Input to short"); //$NON-NLS-1$
	...
	if (!Arrays.equals(checksum, hash))
	    throw new Exception("Checksum does not validate"); //$NON-NLS-1$
