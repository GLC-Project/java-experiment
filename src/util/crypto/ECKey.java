...
import org.mightyfish.asn1.DERInteger;
import org.mightyfish.asn1.DEROctetString;
import org.mightyfish.asn1.DERSequenceGenerator;
import org.mightyfish.asn1.DERTaggedObject;
import org.mightyfish.asn1.DLSequence;
import org.mightyfish.asn1.sec.SECNamedCurves;
...
import org.mightyfish.math.ec.ECFieldElement;
 ...
 * 
 * <p>
 * ECKey also provides access to Litecoin-Qt compatible text message signing, as
...
public class ECKey implements Serializable {
    ...
    static {
	// All clients must agree on the curve to use by agreement. Litecoin
	// uses secp256k1.
	X9ECParameters params = SECNamedCurves.getByName("secp256k1"); //$NON-NLS-1$
    ...
    /**
     * Instance of the KeyCrypter interface to use for encrypting and decrypting
     * the key.
     */
	...
	ECPrivateKeyParameters privParams = (ECPrivateKeyParameters) keypair
		.getPrivate();
	ECPublicKeyParameters pubParams = (ECPublicKeyParameters) keypair
		.getPublic();
	priv = privParams.getD();
	// Unfortunately Bouncy Castle does not let us explicitly change a point
	// to be compressed, even though it
	// could easily do so. We must re-build it here so the ECPoints
	// withCompression flag can be set to true.
	...
	return new ECPoint.Fp(ecParams.getCurve(), uncompressed.getX(),
		uncompressed.getY(), true);
    }

    /**
     * Construct an ECKey from an ASN.1 encoded private key. These are produced
     * by OpenSSL and stored by the Litecoin reference implementation in its
	...
	this(privKeyBytes == null ? null : new BigInteger(1, privKeyBytes),
		pubKey);
    }
     ...
     */
    public ECKey(EncryptedPrivateKey encryptedPrivateKey, byte[] pubKey,
     ...
     * OpenSSL or used by the Litecoin reference implementation in its wallet
     * storage format.
     */
    public byte[] toASN1() {
	try {
	    ...
	    seq.addObject(new DERTaggedObject(0, SECNamedCurves.getByName(
		    "secp256k1").toASN1Primitive())); //$NON-NLS-1$
	...
	b.append("pub:").append(Utils.bytesToHexString(pub)); //$NON-NLS-1$
	if (creationTimeSeconds != 0) {
	    b.append(" timestamp:").append(creationTimeSeconds); //$NON-NLS-1$
	}
	if (isEncrypted()) {
	    b.append(" encrypted"); //$NON-NLS-1$
	}
	return b.toString();
    }
	...
	b.append(toString());
	if (priv != null) {
	    b.append(" priv:").append( //$NON-NLS-1$
		    Utils.bytesToHexString(priv.toByteArray()));
	}
	return b.toString();
    }
    ...
    /**
     * Clears all the ECKey private key contents from memory. WARNING - this
     * method irreversibly deletes the private key information. It turns the
     * ECKEy into a watch only key.
     */
     ...
     * embedded in other data structures in the Litecoin protocol. The raw
	 ...
	 * Litecoin we have to encode them using DER encoding, which is just a
     ...
     * In the Litecoin protocol, they are usually encoded using DER format, so
     * you want {@link com.google.litecoin.core.ECKey.ECDSASignature#toASN1()}
     ...
     * In the Litecoin protocol, they are usually encoded using DER format, so
     * you want
     * {@link com.google.litecoin.core.ECKey.ECDSASignature#encodeToDER()}
     * instead. However sometimes the independent components can be useful, for
     * instance, if you're doing to do further EC maths on them.
     * 
     * @param aesKey
	...
	if (isEncrypted()) {
		...
		throw new KeyCrypterException(
			"This ECKey is encrypted but no decryption key has been supplied."); //$NON-NLS-1$
	    }

	    if (keyCrypter == null) {
		throw new KeyCrypterException(
			"There is no KeyCrypter to decrypt the private key for signing."); //$NON-NLS-1$
	    }

	    privateKeyForSigning = new BigInteger(1, keyCrypter.decrypt(
		    encryptedPrivateKey, aesKey));
	} else {
	    // No decryption of private key required.
	    if (priv == null) {
		throw new KeyCrypterException(
			"This ECKey does not have the private key necessary for signing."); //$NON-NLS-1$
	    } else {
	...
	ECDSASigner signer = new ECDSASigner();
	ECPublicKeyParameters params = new ECPublicKeyParameters(ecParams
	...
	try {
	    ...
	    try {
		return signer.verifySignature(data, r.getPositiveValue(),
			s.getPositiveValue());
	    } catch (NullPointerException e) {
		// Bouncy Castle contains a bug that can cause NPEs given
		// specially crafted signatures. Those signatures
		// are inherently invalid/attack sigs so we just fail them here
		// rather than crash the thread.
		log.error("Caught NPE inside bouncy castle"); //$NON-NLS-1$
		e.printStackTrace();
		return false;
	    }
	} catch (IOException e) {
	...
	try {
	    ...
	    checkArgument(seq.size() == 4,
		    "Input does not appear to be an ASN.1 OpenSSL EC private key"); //$NON-NLS-1$
	    checkArgument(
		    ((DERInteger) seq.getObjectAt(0)).getValue().equals(
			    BigInteger.ONE), "Input is of wrong version"); //$NON-NLS-1$
    ...
    /**
     * Signs a text message using the standard Litecoin messaging signing format
     ...
     *             does not decrypt the ECKey.
     */
    public String signMessage(String message, KeyParameter aesKey)
	    throws KeyCrypterException {
	if (priv == null)
	...
	if (recId == -1)
	    throw new RuntimeException(
		    "Could not construct a recoverable key. This should never happen."); //$NON-NLS-1$
	...
	return new String(Base64.encode(sigData), Charset.forName("UTF-8")); //$NON-NLS-1$
    }

    /**
     * Given an arbitrary piece of text and a Litecoin-format message signature
     ...
     * compatible with the Litecoin-Qt/litecoind format generated by
     ...
     *            The Litecoin-format message signature in base64
	...
	try {
	    ...
	    // decode :(
	    throw new SignatureException("Could not decode base64", e); //$NON-NLS-1$
	}
	// Parse the signature bytes into r/s and the selector value.
	if (signatureEncoded.length < 65)
	    throw new SignatureException(
		    "Signature truncated, expected 65 bytes and got " //$NON-NLS-1$
	...
	if (header < 27 || header > 34)
	    throw new SignatureException("Header byte out of range: " + header); //$NON-NLS-1$
	BigInteger r = new BigInteger(1, Arrays.copyOfRange(signatureEncoded,
		1, 33));
	BigInteger s = new BigInteger(1, Arrays.copyOfRange(signatureEncoded,
		33, 65));
	...
	if (key == null)
	    throw new SignatureException(
		    "Could not recover public key from signature"); //$NON-NLS-1$
	return key;
    }
	...
	if (!Arrays.equals(key.getPubKey(), pub))
	    throw new SignatureException("Signature did not match for message"); //$NON-NLS-1$
    }
    ...
    public static ECKey recoverFromSignature(int recId, ECDSASignature sig,
	    Sha256Hash message, boolean compressed) {
	Preconditions.checkArgument(recId >= 0, "recId must be positive"); //$NON-NLS-1$
	Preconditions.checkArgument(sig.r.compareTo(BigInteger.ZERO) >= 0,
		"r must be positive"); //$NON-NLS-1$
	Preconditions.checkArgument(sig.s.compareTo(BigInteger.ZERO) >= 0,
		"s must be positive"); //$NON-NLS-1$
	...
	if (compressed) {
	    // We have to manually recompress the point as the compressed-ness
	    // gets lost when multiply() is used.
	...
	ECFieldElement alpha = x.multiply(x.square().add(curve.getA())).add(
		curve.getB());
	...
	if (beta.toBigInteger().testBit(0) == yBit) {
	    ...
	    ECFieldElement.Fp y = new ECFieldElement.Fp(curve.getQ(), curve
    ...
    /**
     * Exports the private key in the form used by the Satoshi client
     * "dumpprivkey" and "importprivkey" commands. Use the
     * {@link com.google.litecoin.core.DumpedPrivateKey#toString()} method to
     * get the string.
     * 
     * @param params
     *            The network this key is intended for use on.
	...
	if (newCreationTimeSeconds < 0)
	    throw new IllegalArgumentException(
		    "Cannot set creation time to negative value: " //$NON-NLS-1$
			    + newCreationTimeSeconds);
    ...
    @Override
    public int hashCode() {
	// Public keys are random already so we can just use a part of them as
	// the hashcode. Read from the start to
	// avoid picking up the type code (compressed vs uncompressed) which is
	// tacked on the end.
	return (pub[0] & 0xFF) | ((pub[1] & 0xFF) << 8)
		| ((pub[2] & 0xFF) << 16) | ((pub[3] & 0xFF) << 24);
    }
     ...
     * unchanged. To be secure you need to clear the original, unencrypted
     * private key bytes.
     * 
     * @param keyCrypter
	...
	if (this.keyCrypter != null && !this.keyCrypter.equals(keyCrypter)) {
	    throw new KeyCrypterException(
		    "The keyCrypter being used to decrypt the key is different to the one that was used to encrypt it"); //$NON-NLS-1$
	}
	byte[] unencryptedPrivateKey = keyCrypter.decrypt(encryptedPrivateKey,
		aesKey);
	ECKey key = new ECKey(new BigInteger(1, unencryptedPrivateKey), null,
		isCompressed());
	if (!Arrays.equals(key.getPubKey(), getPubKey()))
	    throw new KeyCrypterException("Provided AES key is wrong"); //$NON-NLS-1$
	return key;
    }
     ...
     * successfully (resulting of loss of all litecoins controlled by the
     * private key) you can use this method to check when you *encrypt* a wallet
     * that it can definitely be decrypted successfully. See
     * {@link Wallet#encrypt(KeyCrypter keyCrypter, KeyParameter aesKey)} for
     * example usage.
     * 
    ...
    public static boolean encryptionIsReversible(ECKey originalKey,
	    ECKey encryptedKey, KeyCrypter keyCrypter, KeyParameter aesKey) {
	String genericErrorText = "The check that encryption could be reversed failed for key " //$NON-NLS-1$
		+ originalKey.toString() + ". "; //$NON-NLS-1$
	try {
	    ECKey rebornUnencryptedKey = encryptedKey.decrypt(keyCrypter,
		    aesKey);
	    if (rebornUnencryptedKey == null) {
		log.error(genericErrorText
			+ "The test decrypted key was missing."); //$NON-NLS-1$
		return false;
	    }
		...
		if (rebornUnencryptedKey.getPrivKeyBytes() == null) {
		    ...
		    if (originalPrivateKeyBytes.length != rebornUnencryptedKey
			    .getPrivKeyBytes().length) {
			log.error(genericErrorText
				+ "The test decrypted private key was a different length to the original."); //$NON-NLS-1$
			return false;
		    } else {
			for (int i = 0; i < originalPrivateKeyBytes.length; i++) {
			    if (originalPrivateKeyBytes[i] != rebornUnencryptedKey
				    .getPrivKeyBytes()[i]) {
				log.error(genericErrorText
					+ "Byte " //$NON-NLS-1$
					+ i
					+ " of the private key did not match the original."); //$NON-NLS-1$
				return false;
			    }
			}
		    }
	...
	return keyCrypter != null && encryptedPrivateKey != null
		&& encryptedPrivateKey.getEncryptedBytes() != null
		&& encryptedPrivateKey.getEncryptedBytes().length > 0;
    }

    /**
     * @return The encryptedPrivateKey (containing the encrypted private key
     *         bytes and initialisation vector) for this ECKey, or null if the
     *         ECKey is not encrypted.
     */
    ...
    /**
     * @return The KeyCrypter that was used to encrypt to encrypt this ECKey.
