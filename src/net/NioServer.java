 ...
 * 
 * <p>
 * <em>This <tt>NioServer</tt> class was copied to this project from its source as
...
public class NioServer {
     ...
     */
    public final static String INPUT_BUFFER_SIZE_PROP = "bufferSize"; //$NON-NLS-1$
     ...
     */
    public final static String OUTPUT_BUFFER_SIZE_PROP = "bufferSize"; //$NON-NLS-1$

    private final static int BUFFER_SIZE_DEFAULT = 4096000;
     ...
     * @see #getState()
     */
    public final static String STATE_PROP = "state"; //$NON-NLS-1$
     ...
     */
    public final static String LAST_EXCEPTION_PROP = "lastException"; //$NON-NLS-1$
    private Throwable lastException;

    private final Collection<NioServer.Listener> listeners = new LinkedList<Listener>(); // Event
    // listeners
    private NioServer.Listener[] cachedListeners = null;
    private final NioServer.Event event = new NioServer.Event(this); // Shared
								     // event
     ...
     */
    public final static String TCP_BINDINGS_PROP = "tcpBindings"; //$NON-NLS-1$
     ...
     */
    public final static String UDP_BINDINGS_PROP = "udpBindings"; //$NON-NLS-1$
     ...
     * @see #getSingleTcpPort()
     */
    public final static String SINGLE_TCP_PORT_PROP = "singleTcpPort"; //$NON-NLS-1$
     ...
     * @see #getSingleUdpPort()
     */
    public final static String SINGLE_UDP_PORT_PROP = "singleUdpPort"; //$NON-NLS-1$
    ...
    private final Set<SocketAddress> pendingTcpAdds = new HashSet<SocketAddress>(); // TCP
    // bindings
    // to
    // add
    // to
    ...
    // next
    // cycle
    private final Set<SocketAddress> pendingUdpAdds = new HashSet<SocketAddress>(); // UDP
    // bindings
    // to
    // add
    // to
    ...
    private final Map<SelectionKey, Boolean> pendingTcpNotifyOnWritable = new HashMap<SelectionKey, Boolean>(); // Turning
														// on
														// and
														// off
														// writable
    ...
    private final Map<SelectionKey, ByteBuffer> leftoverForReading = new HashMap<SelectionKey, ByteBuffer>(); // Store
													      // leftovers
													      // from
													      // the
													      // last
													      // read
    private final Map<SelectionKey, ByteBuffer> leftoverForWriting = new HashMap<SelectionKey, ByteBuffer>(); // Store
													      // leftovers
													      // that
													      // still
													      // need
     ...
     * 
     * @param factory
     *            the ThreadFactory to use when starting the server
     */
	...
	assert seq.charAt(0) == '[' : seq
		+ " Expected '[' at beginning: " + seq.charAt(0); //$NON-NLS-1$
	assert seq.charAt(seq.length() - 1) == ']' : seq
		+ " Expected ']': " + seq.charAt(seq.length() - 1); //$NON-NLS-1$
	for (int i = 1; i < seq.length(); i++) {
	    char c = seq.charAt(i);
	    switch (c) {
	    case '.':
		assert !(PFound && !LFound) : seq
			+ " Between 'P' and 'L' should be 'r': " + c; //$NON-NLS-1$
		assert !undFound : seq + " Should not mix '.' and '_'"; //$NON-NLS-1$
		dotFound = true;
		break;
		...
		assert !(PFound && !LFound) : seq
			+ " Between 'P' and 'L' should be 'r': " + c; //$NON-NLS-1$
		assert !dotFound : seq + " Should not mix '.' and '_'"; //$NON-NLS-1$
		undFound = true;
		break;

	    case 'r':
		assert PFound && !LFound : seq
			+ " Should only see 'r' between 'P' and 'L'"; //$NON-NLS-1$
		rFound = true;
		break;

	    case 'P':
		assert !PFound : seq + " Too many P's"; //$NON-NLS-1$
		if (undFound) {
		    assert buff.position() > 0 : seq
			    + " Expected position to be positive: " + buff; //$NON-NLS-1$
		} else if (!dotFound) {
		    assert buff.position() == 0 : seq
			    + " Expected position to be zero: " + buff; //$NON-NLS-1$
		}
		PFound = true;
		dotFound = false;
		undFound = false;
	    ...
	    case 'L':
		assert PFound : seq + " 'L' should not come before 'P'"; //$NON-NLS-1$
		assert !LFound : seq + " Too many L's"; //$NON-NLS-1$
		if (rFound) {
		    assert buff.position() <= buff.limit() : seq
			    + " Expected possible space between position and limit: " + buff; //$NON-NLS-1$
		} else {
		    assert buff.position() == buff.limit() : seq
			    + " Expected position to equal limit: " + buff; //$NON-NLS-1$
		}
		LFound = true;
		rFound = false;
		break;
	    ...
	    case ']':
		assert PFound && LFound : seq
			+ " 'P' and 'L' not found before ']'"; //$NON-NLS-1$
		assert !endFound : seq + " Too many ]'s"; //$NON-NLS-1$
		if (undFound) {
		    assert buff.limit() < buff.capacity() : seq
			    + " Expected space between limit and capacity: " + buff; //$NON-NLS-1$
		} else if (!dotFound) {
		    assert buff.limit() == buff.capacity() : seq
			    + " Expected limit to equal capacity: " + buff; //$NON-NLS-1$
		}
		endFound = true;
		dotFound = false;
		undFound = false;
		break;
	    default:
		assert false : seq + " Unexpected character: " + c; //$NON-NLS-1$
	...
	if (this.currentState == State.STARTED
		|| this.currentState == State.STARTING) { // Only if already
							  // STARTED
	...
	try {
	    ...
	    ByteBuffer inBuff = ByteBuffer.allocateDirect(this.inputBufferSize); // Buffer
										 // to
										 // use
										 // for
										 // everything
	    ByteBuffer outBuff = ByteBuffer
		    .allocateDirect(this.outputBufferSize); // Buffer to use for
							    // everything
		...
		if (this.selector.select() <= 0) { // Block until notified
		    LOGGER.finer("selector.select() <= 0"); // Possible false start //$NON-NLS-1$
		...
		synchronized (this) {
			...
			try {
			    ...
			    LOGGER.log(
				    Level.SEVERE,
				    "An error occurred while closing the server. " + //$NON-NLS-1$
					    "This try{may have left the server in an undefined state.", //$NON-NLS-1$
				    exc);
			} finally {
			    continue;
			}
		...
		// Possibly resize outBuff if a change was requested since last
		// cycle
		if (this.inputBufferSize != inBuff.capacity()) { // Mismatch
								 // size means
								 // someone
								 // asked for
								 // something
								 // new
		    assert this.inputBufferSize >= 0 : this.inputBufferSize; // We
									     // check
									     // for
									     // this
									     // in
									     // setBufferSize(..)
		    inBuff = ByteBuffer.allocateDirect(this.inputBufferSize); // Resize
									      // and
									      // use
									      // direct
									      // for
		...
		// Possibly resize outBuff if a change was requested since last
		// cycle
		if (this.outputBufferSize != outBuff.capacity()) { // Mismatch
								   // size means
								   // someone
								   // asked for
								   // something
								   // new
		    assert this.outputBufferSize >= 0 : this.outputBufferSize; // We
									       // check
									       // for
									       // this
									       // in
									       // setBufferSize(..)
		    outBuff = ByteBuffer.allocateDirect(this.outputBufferSize); // Resize
										// and
										// use
										// direct
										// for
		...
		Set<SelectionKey> keys = this.selector.selectedKeys(); // These
								       // keys
								       // need
								       // attention
		if (LOGGER.isLoggable(Level.FINEST)) { // Only report this at
						       // finest grained logging
						       // level
		    LOGGER.finest("Keys: " + keys); // Which keys are being examined this round //$NON-NLS-1$
		}

		Iterator<SelectionKey> iter = keys.iterator(); // Iterate over
							       // keys -- cannot
							       // use "for" loop
							       // since we
							       // remove keys
		while (iter.hasNext()) { // Each accKey
			...
			if (key.isAcceptable()) { // New, incoming connection?
			    handleAccept(key, outBuff); // Handle accepting
							// connections
			}
			...
			else if (key.isReadable()) { // Existing connection has
						     // data (or is closing)
			...
			// This could be an ongoing TCP connection
			// or a new (is there any other kind) UDP datagram
			else if (key.isWritable()) { // Existing connection has
						     // data (or is closing)
			    handleWrite(key, inBuff, outBuff); // Handle data
			} // end if: readable

		    } catch (IOException exc) {
			LOGGER.warning("Encountered an error with a connection: " + exc.getMessage()); //$NON-NLS-1$
			fireExceptionNotification(exc);
			key.channel().close();
		    } // end catch
		...
		if (this.currentState == State.STOPPING) { // User asked to stop
			...
			this.selector.close();
			this.selector = null;
			LOGGER.info("Server closed normally."); //$NON-NLS-1$
		    } catch (IOException exc2) {
			this.lastException = exc2;
			LOGGER.log(
				Level.SEVERE,
				"An error occurred while closing the server. " + //$NON-NLS-1$
					"This may have left the server in an undefined state.", //$NON-NLS-1$
				exc2);
		    ...
		    LOGGER.log(
			    Level.WARNING,
			    "Server closed unexpectedly: " + exc.getMessage(), exc); //$NON-NLS-1$
		} // end else
	    } // end sync
	    fireExceptionNotification(exc);
	} finally {
     ...
     *             if something within throws it
     */
    private synchronized boolean runLoopCheck() throws IOException {

	if (this.currentState == State.STOPPING) {
	    LOGGER.finer("Stopping server by request."); //$NON-NLS-1$
	...
	for (SocketAddress addr : this.pendingTcpAdds) { // For each add
	    LOGGER.fine("Binding TCP: " + addr); //$NON-NLS-1$
	    ServerSocketChannel sc = ServerSocketChannel.open(); // Open a
								 // channel
	    sc.socket().bind(addr); // Bind as requested
	    sc.configureBlocking(false); // Make non-blocking
	    SelectionKey acceptKey = sc.register( // Register with master
						  // Selector
		    this.selector, SelectionKey.OP_ACCEPT); // We want to
							    // "accept"
							    // connections
	...
	// Pending UDP Adds
	for (SocketAddress addr : this.pendingUdpAdds) { // Same comments as for
							 // TCP
	    LOGGER.fine("Binding UDP: " + addr); //$NON-NLS-1$
	    DatagramChannel dc = DatagramChannel.open();
	    dc.socket().bind(addr);
	    dc.configureBlocking(false);
	    SelectionKey acceptKey = dc.register(this.selector,
		...
		try {
		    ...
		    // CLEANUP: multicast support for NIO
		    // create a temporary instanceof PlainDatagramSocket, set
		    // its fd and configure it
		    @SuppressWarnings(value = "unchecked")
		    Constructor<? extends DatagramSocketImpl> c = (Constructor<? extends DatagramSocketImpl>) Class
			    .forName("java.net.PlainDatagramSocketImpl").getDeclaredConstructor(); //$NON-NLS-1$
		    c.setAccessible(true);
		    DatagramSocketImpl socketImpl = c.newInstance();
		    Field channelFd = Class
			    .forName("sun.nio.ch.DatagramChannelImpl").getDeclaredField("fd"); //$NON-NLS-1$ //$NON-NLS-2$
		    channelFd.setAccessible(true);
		    Field socketFd = DatagramSocketImpl.class
			    .getDeclaredField("fd"); //$NON-NLS-1$
		    socketFd.setAccessible(true);
		    socketFd.set(socketImpl, channelFd.get(dc));
		    try {
			Method m = DatagramSocketImpl.class
				.getDeclaredMethod(
					"joinGroup", SocketAddress.class, NetworkInterface.class); //$NON-NLS-1$
			...
			// important, otherwise the fake socket's finalizer will
			// nuke the fd
			socketFd.set(socketImpl, null);
		    }
		} catch (Exception ex) {
		    LOGGER.warning("Experimental feature failed. Could not join multicast group: " + ex.getMessage()); //$NON-NLS-1$
	...
	// Pending TCP Removes
	for (Map.Entry<SocketAddress, SelectionKey> e : this.pendingTcpRemoves
		.entrySet()) {
	...
	// Pending UDP Removes
	for (Map.Entry<SocketAddress, SelectionKey> e : this.pendingUdpRemoves
		.entrySet()) {
	    SelectionKey key = e.getValue(); // Get the registered accKey
	    if (key != null) { // Might be null if someone gave us bogus address
		key.channel().close(); // Close the channel
	...
	for (Map.Entry<SelectionKey, Boolean> e : this.pendingTcpNotifyOnWritable
		.entrySet()) {
	    SelectionKey key = e.getKey();
	    if (key != null && key.isValid()) {
		int ops = e.getKey().interestOps(); // Current ops
		if (e.getValue().booleanValue()) { // Notify?
	...
	this.pendingTcpNotifyOnWritable.clear(); // Remove from list of pending
						 // changes
     ...
     * 
     * @param sel
     *            The selector with which we'll register
     * @param key
     *            The OP_ACCEPT accKey
     * @throws java.io.IOException
     *             if an error occurs
     */
    @SuppressWarnings("null")
    private void handleAccept(SelectionKey accKey, ByteBuffer outBuff)
	    throws IOException {
	assert accKey.isAcceptable() : accKey.readyOps(); // We know it should
							  // be acceptable
	assert this.selector.isOpen(); // Not sure this matters. Meh.

	SelectableChannel sc = accKey.channel(); // Channel for th accKey
	assert sc instanceof ServerSocketChannel : sc; // Only our TCP
						       // connections have
						       // OP_ACCEPT

	ServerSocketChannel ch = (ServerSocketChannel) accKey.channel(); // Server
									 // channel
	SocketChannel incoming = null; // Reusable for all pending connections
	while ((incoming = ch.accept()) != null) { // Iterate over all pending
						   // connections
	    incoming.configureBlocking(false); // Non-blocking IO
	    SelectionKey incomingReadKey = incoming.register( // Register new
							      // connection
		    this.selector, // With the Selector
		    SelectionKey.OP_READ | SelectionKey.OP_WRITE); // Want to
								   // READ and
								   // write data
	    ...
	    // ////// FIRE EVENT ////////
	    fireNewConnection(incomingReadKey, outBuff); // Fire new connection
							 // event
	    // ////// FIRE EVENT ////////

	    // If there are leftovers, save them for next
	    // time the channel is ready.
	    if (outBuff.remaining() > 0) { // Did the user leave data to be
					   // written?
		ByteBuffer leftover = ByteBuffer.allocateDirect(outBuff
			.remaining()); // Create/resize
		leftover.put(outBuff).flip(); // Save new leftovers
		assert knownState(leftover, "[PrrL..]"); //$NON-NLS-1$
		this.leftoverForWriting.put(incomingReadKey, leftover); // Save
									// leftovers
									// for
									// next
									// time
		this.setNotifyOnWritable(incomingReadKey, true); // Notify that
								 // we have
								 // something to
								 // write
		...
		LOGGER.finest("  " + incoming + ", key: " + incomingReadKey); //$NON-NLS-1$ //$NON-NLS-2$
	    }
     ...
     * 
     * @param key
     *            The accKey associated with the reading
     * @param buff
     *            the ByteBuffer to hold the data
     * @throws java.io.IOException
     *             if an error occurs
     */
    private void handleRead(SelectionKey key, ByteBuffer inBuff,
	    ByteBuffer outBuff) throws IOException {

	SelectableChannel sc = key.channel();
	inBuff.clear(); // Clear input outBuff
	assert knownState(inBuff, "[PrrrL]"); //$NON-NLS-1$

	// TCP
	if (sc instanceof SocketChannel) {

	    SocketChannel client = (SocketChannel) key.channel(); // Source
								  // socket
	    ByteBuffer leftoverR = this.leftoverForReading.get(key); // Leftover
								     // data
								     // from
								     // last
								     // read
	    ...
	    if (leftoverR != null && leftoverR.remaining() > 0) { // Have a
								  // leftoverR
								  // outBuff
		assert knownState(leftoverR, "[..PrrL..]"); //$NON-NLS-1$

		if (leftoverR.remaining() <= inBuff.remaining()) { // Is inBuff
								   // big enough
								   // to hold
								   // leftover
		    inBuff.put(leftoverR); // Preload all of leftoverR
		    assert knownState(inBuff, "[..PrrL]"); //$NON-NLS-1$

		} else {
		    while (leftoverR.hasRemaining() && inBuff.hasRemaining()) { // While
										// they
										// both
										// have
										// more
										// to
										// copy
			inBuff.put(leftoverR.get()); // Copy one byte at a time.
						     // Yuck.
		    } // end while
		    assert knownState(inBuff, "[..PL]"); //$NON-NLS-1$
		} // end else
		...
		if (LOGGER.isLoggable(Level.FINER)) {
		    LOGGER.finer("Connection closed: " + key); //$NON-NLS-1$
		}
		...
		assert knownState(inBuff, "[PrrL..]"); //$NON-NLS-1$
		assert knownState(outBuff, "[PL...]"); //$NON-NLS-1$

		fireTcpDataReceived(key, inBuff, outBuff); // Fire event for new
							   // data
		...
		if (outBuff.remaining() > 0) { // Did the user leave data to be
					       // written?
		    ByteBuffer leftoverW = this.leftoverForWriting.get(key); // Leftover
									     // still
									     // to
									     // be
									     // written
		    if (leftoverW == null) { // Leftover outBuff not yet
					     // created?
			leftoverW = ByteBuffer.allocateDirect(outBuff
				.remaining()); // Create/resize
		    } else {
			assert knownState(outBuff, "[..PrrL..]"); //$NON-NLS-1$
			if (outBuff.remaining() > // Amount requested to be
						  // written
			leftoverW.capacity() - leftoverW.limit()) { // vs. space
								    // remaining
								    // in
								    // leftoverW

			    ByteBuffer temp = ByteBuffer.allocateDirect( // Resize
									 // outBuff
									 // to...
				    leftoverW.limit() - leftoverW.position() + // data
									       // remaining
									       // in
									       // leftover
									       // plus
					    outBuff.remaining()); // amount
								  // requested
								  // this round

			    temp.put(leftoverW).flip(); // Put new buff in
							// proper state (ready
							// to be read)
			    leftoverW = temp; // Replace old leftoverW outBuff
			    assert knownState(leftoverW, "[PrrL__]"); //$NON-NLS-1$

			} // end if: resize leftoverW
			assert knownState(leftoverW, "[PrrL..]"); //$NON-NLS-1$
			leftoverW.position(leftoverW.limit()); // Move position
							       // to end of
							       // relevant data
			leftoverW.limit(leftoverW.capacity()); // Move limit to
							       // end of outBuff
			assert knownState(leftoverW, "[..PrrL]"); //$NON-NLS-1$
		    }
		    assert knownState(leftoverW, "[..PrrL]"); //$NON-NLS-1$
		    assert knownState(outBuff, "[..PrrL..]"); //$NON-NLS-1$
		    leftoverW.put(outBuff).flip(); // Record newly-arrived data
						   // and flip leftoverW

		    assert knownState(leftoverW, "[PrrL..]"); //$NON-NLS-1$
		    assert knownState(outBuff, "[..PL..]"); //$NON-NLS-1$

		    this.leftoverForWriting.put(key, leftoverW); // Save
								 // leftovers
								 // for next
								 // time
		    this.setNotifyOnWritable(key, true); // Make sure server
							 // processes writes
		} // end if: has remaining bytes

		// If there's leftover data that wasn't read,
		// save that for next time the event is fired.
		if (inBuff.remaining() > 0) { // Did the user leave data for
					      // next time?
		    if (leftoverR == null || // Leftover outBuff not yet
					     // created?
			    inBuff.remaining() > leftoverR.capacity()) { // Or
									 // is
									 // too
									 // small?
			leftoverR = ByteBuffer.allocateDirect(inBuff
				.remaining()); // Create/resize
		...
		this.leftoverForReading.put(key, leftoverR); // Save leftovers
							     // for next time
	    } // end else: read
	    client.close();
	...
	// Datagram
	    ...
	    DatagramChannel dc = (DatagramChannel) sc; // Cast to datagram
						       // channel
	    SocketAddress remote = null;
	    while ((remote = dc.receive(inBuff)) != null) { // Loop over all
							    // pending datagrams
     ...
     * 
     * @param key
     *            The accKey associated with the writing
     * @param buff
     *            the ByteBuffer to hold the data
     * @throws java.io.IOException
     *             if an error occurs
    ...
    private void handleWrite(SelectionKey key,
	    @SuppressWarnings("unused") ByteBuffer inBuff, ByteBuffer outBuff)
	    throws IOException {
	...
	ByteBuffer leftover = this.leftoverForWriting.get(key); // Leftover data
								// still needing
								// to be written
	if (leftover != null && leftover.remaining() > 0) { // Have a leftoverR
							    // outBuff
	    assert knownState(leftover, "[PrrL..]"); //$NON-NLS-1$
	...
	if (leftover == null || !leftover.hasRemaining()) {
	    ...
	    // ////// FIRE EVENT ////////
	    fireTcpReadyToWrite(key, outBuff); // Notify listeners who will load
					       // outBuff
	    // ////// FIRE EVENT ////////
	    ...
	    // time the channel is ready.
	    if (outBuff.hasRemaining()) { // Is there _still_ data left to
					  // write?
		if (leftover == null || // Leftover outBuff not yet created?
			outBuff.remaining() > leftover.capacity()) { // Or is
								     // too
								     // small?
	    ...
	    this.leftoverForWriting.put(key, leftover); // Save leftovers for
							// next time
	...
	if (this.closeAfterWriting.contains(key) && // Has user requested
						    // "close after writing?"
		(leftover == null || !leftover.hasRemaining())) { // And is
								  // there
								  // nothing
								  // left to
								  // write?
     ...
     * 
     * @param key
     *            The accKey representing the connection in question
     * @param notify
     *            Whether or not to notify
     * @throws NullPointerException
     *             if accKey is null
     */
    public synchronized void setNotifyOnWritable(SelectionKey key,
	    boolean notify) {
	// if(notify){
	...
	if (key == null) {
	    throw new NullPointerException(
		    "Cannot set notifications for null key."); //$NON-NLS-1$
	}
	this.pendingTcpNotifyOnWritable.put(key, Boolean.valueOf(notify));
	if (this.selector != null) {
	    this.selector.wakeup();
	}
    }
     ...
     * 
     * @param key
     *            the SelectionKey for the corresponding connection
     */
	...
	if (size <= 0) {
	    throw new IllegalArgumentException(
		    "New buffer size must be positive: " + size); //$NON-NLS-1$
	...
	firePropertyChange(INPUT_BUFFER_SIZE_PROP, Integer.valueOf(oldVal),
	...
	firePropertyChange(OUTPUT_BUFFER_SIZE_PROP, Integer.valueOf(oldVal),
		Integer.valueOf(size));
    }
     ...
     * 80: <code>addTcpBinding( new InetAddress(80) );</code>. The server can
	...
	Set<SocketAddress> oldVal = this.getTcpBindings(); // Save old set for
							   // prop change event
	this.tcpBindings.put(addr, null); // Add binding
	Set<SocketAddress> newVal = this.getTcpBindings(); // Save new set for
							   // prop change event
	this.pendingTcpAdds.add(addr); // Prepare pending add action
	this.pendingTcpRemoves.remove(addr); // In case it's also pending a
					     // remove

	if (this.selector != null) { // If there's a selector...
	    this.selector.wakeup(); // Wake it up to handle the add action
	}
	firePropertyChange(TCP_BINDINGS_PROP, oldVal, newVal); // Fire prop
							       // change
	return this;
    }
	...
	Set<SocketAddress> oldVal = this.getTcpBindings(); // Save old set for
							   // prop change event
	this.pendingTcpRemoves.put(addr, this.tcpBindings.get(addr)); // Prepare
								      // pending
								      // remove
								      // action
     ...
     * ports and on which interfaces you want the server to listen. In the
     * simplest case, you might do the following to listen generically on port
     * 6997: <code>addUdpBinding( new InetAddress(6997) );</code>. The server
     * can listen on multiple ports at once.
     * 
     * @param addr
     *            The address on which to listen
     * @return "this" to aid in chaining commands
    ...
    /**
     * <p>
     * <em>Experimental Hack</em> - Adds a UDP binding to the server and joins
     * the given multicast group (if <code>group<code>
     ...
     * gentleman's website (<a
     * href="http://www.mernst.org/blog/archives/12-01-2006_12-31-2006.html"
     * >http://www.mernst.org/blog/archives/12-01-2006_12-31-2006.html</a>) that
     * makes multicast work -- for now.
     * </p>
     * 
     * @param addr
     *            The address on which to listen
	...
	Map<SocketAddress, String> oldVal = this.getUdpBindings(); // Save old
								   // set for
								   // prop
								   // change
								   // event
	this.pendingUdpRemoves.put(addr, this.udpBindings.get(addr)); // Prepare
								      // pending
								      // remove
								      // action
	...
	Map<SocketAddress, String> newVal = this.getUdpBindings(); // Save new
								   // set for
								   // prop
								   // change
								   // event
	...
	firePropertyChange(UDP_BINDINGS_PROP, oldVal, newVal); // Fire prop
							       // change
	return this;
    }
     ...
     * @return "this" to aid in chaining commands
     */
    public synchronized NioServer setUdpBindings(
	    Map<SocketAddress, String> newMap) {
	...
	int newVal = port;
	firePropertyChange(SINGLE_TCP_PORT_PROP, Integer.valueOf(oldVal),
	...
	int newVal = port;
	firePropertyChange(SINGLE_UDP_PORT_PROP, Integer.valueOf(oldVal),
		Integer.valueOf(newVal));
	return this;
    }
     ...
     * 
     * @param key
     *            the SelectionKey associated with the data
     * @param outBuff
     *            the outBuff containing the new (and possibly leftoverR) data
     */
    protected synchronized void fireTcpDataReceived(SelectionKey key,
	    ByteBuffer inBuff, ByteBuffer outBuff) {

	if (this.cachedListeners == null) {
	    this.cachedListeners = this.listeners
		    .toArray(new NioServer.Listener[this.listeners.size()]);
	}
	...
	for (NioServer.Listener l : this.cachedListeners) {
	    try {
		l.tcpDataReceived(this.event);
	    } catch (Exception exc) {
		LOGGER.warning("NioServer.Listener " + l + " threw an exception: " + exc.getMessage()); //$NON-NLS-1$ //$NON-NLS-2$
		fireExceptionNotification(exc);
	    } // end catch
     ...
     *            the outBuff containing the new (and possibly leftoverR) data
     */
    protected synchronized void fireTcpReadyToWrite(SelectionKey key,
	    ByteBuffer outBuff) {
	assert knownState(outBuff, "[PL..]"); //$NON-NLS-1$
     ...
     * @param remote
     *            the source address of the datagram or null if not available
     * @param outBuff
     *            the output buffer for writing data
     */
    protected synchronized void fireUdpDataReceived(SelectionKey key,
	    ByteBuffer inBuff, ByteBuffer outBuff, SocketAddress remote) {
     ...
     * 
     * @param key
     *            The accKey for the closed connection.
     */
     ...
     * 
     * @param key
     *            the SelectionKey associated with the connection
     */
	...
	for (NioServer.Listener l : this.cachedListeners) {
	    try {
		l.newConnectionReceived(this.event);
	    } catch (Exception exc) {
		LOGGER.warning("NioServer.Listener " + l + " threw an exception: " + exc.getMessage()); //$NON-NLS-1$ //$NON-NLS-2$
		fireExceptionNotification(exc);
	    } // end catch
	...
	firePropertyChange(INPUT_BUFFER_SIZE_PROP, null,
		Integer.valueOf(getInputBufferSize()));
	firePropertyChange(OUTPUT_BUFFER_SIZE_PROP, null,
		Integer.valueOf(getOutputBufferSize()));
    }
	...
	try {
	    ...
	    LOGGER.log(
		    Level.WARNING,
		    "A property change listener threw an exception: " + exc.getMessage() //$NON-NLS-1$
		    , exc);
	    fireExceptionNotification(exc);
	} // end catch
    } // end fire
     ...
     * 
     * <p>
     * Of critical importance are the input and output buffers, as provided by
     ...
     * <tr>
     * <th>Event</th>
     * <th>Input outBuff upon entering event</th>
     * <th>Output outBuff upon exiting event</th>
     * </tr>
     * </thead> <tbody>
     * <tr>
	 ...
	 * with the event (with <code>OP_READ</code> interest), is the accKey
	 * that will be used with the data received event. In this way, you can
	 * seed a <code>Map</code> or other data structure and associate this
	 * very accKey with the connection. You will only get new connection
	 * events for TCP connections (not UDP).
	 * </p>
	 * 
	 * <p>
	 * The accKey's attachment mechanism is unused by NioServer and is
	 * available for you to store whatever you like.
	 * </p>
	 * 
	 * <p>
	 * If your protocol requires the server to respond to a client upon
	 * connection, this sample code demonstrates such an arrangement being
	 * careful not to instantiate objects at each event:
	 * </p>
	 * 
	 * <pre>
	 ...
	 * with {@link NioServer.Event#getInputBuffer()}. This is the source
	 * ByteBuffer used by the server directly to receive the data. It is a
	 * "direct" ByteBuffer (created with
	 * <code>ByteBuffer.allocateDirect(..)</code>).
	 * </p>
	 * 
	 * <p>
	 * Read from it as much as you can. Any data that remains on or after
	 * the value of <code>position()</code> will be saved for the next time
	 * an event is fired. In this way, you can defer processing incomplete
	 * data until everything arrives.
	 * </p>
	 * 
	 * <p>
	 * <em>If you leave the buffer full, all the way up to the
	 * very last byte at <code>capacity()</code>, then that
	 * connection cannot receive any more data.</em> On some operating
	 * systems and Java virtual machines, this will serve to throttle back
	 ...
	 * 
	 * <p>
	 * If you wish to also write data as a result of what is read, the
	 * preferred method is to retrieve the output buffer with
	 * {@link NioServer.Event#getOutputBuffer()} and write bytes there,
	 * leaving the buffer in a ready-to-read state:
	 * </p>
	 * 
	 * <pre>
	 * ByteBuffer buff = evt.getOutputBuffer(); // Reuse it since it's already
	 * // allocated
	 ...
	 * 
	 * <p>
	 * To make a trivial "echo" program, simple copy the contents of the
	 ...
	 * port in the datagram. Your firewall mileage may vary.
	 * </p>
	 * 
	 * @param evt
	 *            the shared event
	...
	/**
	 * <p>
	 * Fired when a TCP channel is ready to be written to. The preferred way
	 * to write is to leave data in the output buffer (
	 * {@link NioServer.Event#getOutputBuffer()}) attached to the event. The
	 * server will take care of sending it even if it can't all be sent at
	 * once (perhaps a slow network connection).
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * You can also just retrieve the accKey's channel, cast it to a
	 ...
	 * 
	 * <p>
	 * Be aware of how large the output buffer is. You can change the output
	 * buffer size that the server uses with the
	...
	public void tcpDataReceived(
		@SuppressWarnings("unused") NioServer.Event evt) {// Empty
	}
	 ...
	 */
	@Override
	public void newConnectionReceived(
		@SuppressWarnings("unused") NioServer.Event evt) {// Empty
	}
	 ...
	 * 
	 * @param key1
	 *            The SelectionKey for the event
	 * @param remoteUdp1
	 *            the remote UDP source or null for TCP
	 */
	protected void reset(SelectionKey key1, ByteBuffer inBuff1,
		ByteBuffer outBuff1, SocketAddress remoteUdp1) {
	    this.key = key1;
	    this.inBuff = inBuff1;
	    this.outBuff = outBuff1;
	    this.remoteUdp = remoteUdp1;
	}

	/**
	 * <p>
	 * Returns the {@link java.nio.ByteBuffer} that contains the incoming
	 * data for this connection. Read from it as much as you can. Any data
	 * that remains on or after the value of <code>position()</code> will be
	 * saved for the next time an event is fired. In this way, you can defer
	 * processing incomplete data until everything arrives. This applies to
	 * the {@link NioServer.Listener#tcpDataReceived(NioServer.Event)}
	 * event.
	 * </p>
	 * 
	 * <p>
	 * Example: You are receiving lines of text. The ByteBuffer returned
	...
	/**
	 * <p>
	 * Returns the {@link java.nio.ByteBuffer} in which you leave data to be
	 * written to the client. This applies to the
	 * {@link NioServer.Listener#newConnectionReceived(NioServer.Event)},
	 * {@link NioServer.Listener#tcpDataReceived(NioServer.Event)}, and
	 * {@link NioServer.Listener#tcpReadyToWrite(NioServer.Event)} events.
	 * </p>
	 * 
	 * @return outBuff with the data
	 */
	 ...
	 * </p>
	 * <p>
	 * This is essentially a convenience method for returning the same-name
	 * methods from the accKey's channel after checking the type of channel
	 * (SocketChannel or DatagramChannel).
	 * </p>
	 * 
	...
	public SocketAddress getLocalSocketAddress() {
		...
		if (sc instanceof SocketChannel) {
		    addr = ((SocketChannel) sc).socket()
			    .getLocalSocketAddress();
		} else if (sc instanceof DatagramChannel) {
		    addr = ((DatagramChannel) sc).socket()
			    .getLocalSocketAddress();
		}
	    }
	    return addr;
	...
	public boolean isTcp() {
	    return this.key == null ? false
		    : this.key.channel() instanceof SocketChannel;
	}
	...
	public boolean isUdp() {
	    return this.key == null ? false
		    : this.key.channel() instanceof DatagramChannel;
	}
	 ...
	 * 
	 * @param notify
	 *            whether or not to provide notifications.
	 */
