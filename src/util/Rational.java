...
/**
 * Fractions (rational numbers). They are divisions of two BigInteger variables,
 * reduced to greatest common divisors of 1.
 * 
...
class Rational implements Cloneable {
     ...
     */
    static BigInteger MAX_INT = new BigInteger("2147483647"); //$NON-NLS-1$
    static BigInteger MIN_INT = new BigInteger("-2147483648"); //$NON-NLS-1$
     ...
     *            numerator with implicit denominator equal to 1.
     ...
     * @warning this does not yet test for a denominator equal to zero 5
     */
	...
	if (exponent.compareTo(MAX_INT) == 1) {
	    throw new NumberFormatException(
		    "Exponent " + exponent.toString() + " too large.");
	}
	if (exponent.compareTo(MIN_INT) == -1) {
	    throw new NumberFormatException(
		    "Exponent " + exponent.toString() + " too small.");
	}
	 ...
	 * first in a BigDecimal environment and converst the result.
	 */
	BigDecimal adivb = (new BigDecimal(a)).divide(new BigDecimal(b),
