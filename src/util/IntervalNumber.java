...
/**
 * Реализация полной интервальной арифметики Каухера. Пока без правильных
 * округлений и проверки правильности создания интервалов.
 * 
...
public class IntervalNumber implements AbstractNumber {
    ...
    private static MathContext mcHigh = new MathContext(9,
	...
	if (this instanceof IntervalNumber && _b instanceof IntervalNumber) {
	    ...
	    return new IntervalNumber(a.low.add(b.low, mcLow),
		    a.high.add(b.high, mcHigh));
	}
	...
	if (this instanceof IntervalNumber && _b instanceof IntervalNumber) {
	    IntervalNumber a = this;
	    IntervalNumber b = (IntervalNumber) _b;
	    return new IntervalNumber(a.low.subtract(b.high, mcLow),
		    a.high.subtract(b.low, mcLow));
	}
	throw new ArithmeticException("Unexpected arithmetical error!!!");
    }
	...
	if (this instanceof IntervalNumber && _b instanceof IntervalNumber) {
		...
		case P: {
		    return new IntervalNumber(a.low.multiply(b.low, mcLow),
			    a.high.multiply(b.high, mcHigh));
		}
		case Z: {
		    return new IntervalNumber(a.high.multiply(b.low, mcLow),
			    a.high.multiply(b.high, mcHigh));
		}
		case negP: {
		    return new IntervalNumber(a.high.multiply(b.low, mcLow),
			    a.low.multiply(b.high, mcHigh));
		}
		case dualZ: {
		    return new IntervalNumber(a.low.multiply(b.low, mcLow),
			    a.low.multiply(b.high, mcHigh));
		...
		switch (b.getTypeOfInterval()) {
		case P: {
		    return new IntervalNumber(a.low.multiply(b.high, mcLow),
			    a.high.multiply(b.high, mcHigh));
		}
		case Z: {
		    return new IntervalNumber(
			    a.low.multiply(b.high, mcLow)
				    .min(a.high.multiply(b.low, mcLow)),
			    a.low.multiply(b.low, mcHigh)
				    .max(a.high.multiply(b.high, mcHigh)));
		}
		case negP: {
		    return new IntervalNumber(a.high.multiply(b.low, mcLow),
			    a.low.multiply(b.low, mcHigh));
		...
		case negP: {
		    return new IntervalNumber(a.high.multiply(b.high, mcLow),
			    a.low.multiply(b.low, mcHigh));
		}
		case dualZ: {
		    return new IntervalNumber(a.high.multiply(b.high, mcLow),
		...
		case dualZ: {
		    return new IntervalNumber(
			    a.low.multiply(b.low, mcLow)
				    .max(a.high.multiply(b.high, mcLow)),
			    a.low.multiply(b.high, mcHigh)
				    .min(a.high.multiply(b.low, mcHigh)));
		}
		}
	    }
	}
	...
	if (this instanceof IntervalNumber && _b instanceof IntervalNumber) {
	    ...
	    IntervalNumber b = (IntervalNumber) _b;
	    if (!b.zeroInPro())
		return new IntervalNumber(mult(
			new IntervalNumber(BigDecimal.ONE.divide(b.high, mcLow),
				BigDecimal.ONE.divide(b.low, mcHigh))));
	    else
    ...
    /*
     * @Override public AbstractNumber sqrt() { switch (getTypeOfInterval()){
     * case P: {return new IntervalNumber(low, low);} case Z: {} case negP: {}
     * case dualZ: {throw new ArithmeticException(
     * "Operation sqrt not defined at Z, negP & dualZ");} } throw new
     * ArithmeticException("Unexpected arithmetical exception"); // throw new
	...
	if (obj instanceof IntervalNumber && this instanceof IntervalNumber) {
	    ...
	    return Math
		    .abs(thisInterval.low.doubleValue()
			    - interval.low.doubleValue()) <= 1e-7
		    && Math.abs(thisInterval.high.doubleValue()
			    - interval.high.doubleValue()) <= 1e-7;
	}
	return false;
    }
	...
	if (low.compareTo(BigDecimal.ZERO) <= 0
		&& 0 <= high.compareTo(BigDecimal.ZERO))
	    return typeOfInterval.Z;
	else if (low.compareTo(BigDecimal.ZERO) >= 0
		&& high.compareTo(BigDecimal.ZERO) >= 0)
	    return typeOfInterval.P;
	else if (low.compareTo(BigDecimal.ZERO) <= 0
		&& high.compareTo(BigDecimal.ZERO) <= 0)
	    return typeOfInterval.negP;
	else if (high.compareTo(BigDecimal.ZERO) <= 0
		&& 0 <= low.compareTo(BigDecimal.ZERO))
	...
	return new IntervalNumber(BigDecimal.ONE.divide(low, mcLow),
		BigDecimal.ONE.divide(high, mcHigh));
    };

    public IntervalNumber innerSub(IntervalNumber y) {
	return new IntervalNumber(low.subtract(y.low, mcLow),
		high.subtract(y.high, mcHigh));
    }

    public boolean zeroInPro() { // "Новый" сособ сравнения
	IntervalNumber y = pro();
	return y.low.compareTo(BigDecimal.ZERO) <= 0
		&& 0 <= y.high.compareTo(BigDecimal.ZERO);
    }
	...
	return new IntervalNumber(x.add(
		new IntervalNumber(y.high.negate(mcLow), y.low.negate(mcLow))));
    }
}
