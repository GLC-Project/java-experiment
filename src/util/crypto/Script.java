 ...
 *
 * <p>
 * Feathercoin transactions don't specify what they do directly. Instead <a
 * href="https://en.feathercoin.it/wiki/Script">a small binary stack
 * language</a> is used to define programs that when evaluated return whether
 * the transaction "accepts" or rejects the other transactions connected to it.
 * </p>
 *
 * <p>
 ...
 * methods for building scripts.
 * </p>
 */
public class Script {
    // Some constants used for decoding the scripts, copied from the reference
    // client
    // push value
    ...
    // The program is a set of byte[]s where each element is either [opcode] or
    // [data, data, data ...]
     ...
     * @param offset
     *            How many bytes into programBytes to start reading from.
     * @param length
     *            How many bytes to read.
     * @throws ScriptException
     */
    public Script(NetworkParameters params, byte[] programBytes, int offset,
	    int length) throws ScriptException {
	this.params = params;
    ...
    /**
     * Converts the given OpCode into a string (eg "0", "PUSHDATA", or
     * "NON_OP(10)")
     */
	...
	try {
	    ...
	    throw new ScriptException(
		    "Attempted to read outside of script boundaries");
	}
    }
	...
	offset = 0;
	chunks = new BigFastList<ScriptChunk>(10); // Arbitrary choice of initial
						 // size.
	cursor = offset;
	while (cursor < offset + length) {
		...
		chunks.add(new ScriptChunk(false, getData(opcode),
			startLocationInProgram)); // opcode == len here.
		...
		// Though this is allowed, because its value cannot be > 520, it
		// should never actually be used
		long len = readByte() | (readByte() << 8) | (readByte() << 16)
			| (readByte() << 24);
    ...
    /**
     * Returns true if this script is of the form <sig> OP_CHECKSIG. This form
     * was originally intended for transactions where the peers talked to each
     * other directly via TCP/IP, but has fallen out of favor with time due to
     * that mode of operation being susceptible to man-in-the-middle attacks. It
     * is still used in coinbase outputs and can be useful more exotic types of
     * transaction, but today most payments are to addresses.
     */
    ...
    /**
     * Returns true if this script is of the form DUP HASH160 <pubkey hash>
     * EQUALVERIFY CHECKSIG, ie, payment to an address like
     * 1VayNert3x1KzbpzMGt2qdqrAThiRovi8. This form was originally intended for
     * the case where you wish to send somebody money with a written code
     * because their node is offline, but over time has become the standard way
     * to make payments due to the short and recognizable base58 form addresses
     * come in.
     */
	...
	// Otherwise, the third element is the hash of the public key, ie the
	// feathercoin address.
	return chunks.get(2).data;
    }

    /**
     * Returns the public key in this script. If a script contains two constants
     * and nothing else, it is assumed to be a scriptSig (input) for a
     * pay-to-address output and the second constant is returned (the first is
     * the signature). If a script contains a constant and an OP_CHECKSIG
     * opcode, the constant is returned as it is assumed to be a direct
     * pay-to-key scriptPubKey (output) and the first constant is the public
     * key.
     *
     * @throws ScriptException
	...
	if (chunks.size() != 2) {
	    throw new ScriptException(
		    "Script not of right size, expecting 2 but got "
			    + chunks.size());
	}
	if (chunks.get(0).data.length > 2 && chunks.get(1).data.length > 2) {
	    // If we have two large constants assume the input to a
	    // pay-to-address output.
	    return chunks.get(1).data;
	} else if (chunks.get(1).data.length == 1
		&& chunks.get(1).equalsOpCode(OP_CHECKSIG)
		&& chunks.get(0).data.length > 2) {
	    // A large constant followed by an OP_CHECKSIG is the key.
	    return chunks.get(0).data;
	} else {
	    throw new ScriptException("Script did not match expected form: "
		    + toString());
	}
    }
     ...
     * @throws Exception
     */
    public Address getFromAddress() throws Exception {
     ...
     * @throws Exception
     */
    public Address getToAddress() throws Exception {
     ...
     * opcode prefix To write an integer call writeBytes(out,
     * Utils.reverseBytes(Utils.encodeMPI(val, false)));
     */
	...
	try {
	    // TODO: Do this by creating a Script *first* then having the script
	    // reassemble itself into bytes.
	    ByteArrayOutputStream bits = new ByteArrayOutputStream(24);
	    bits.write(OP_DUP);
    ...
    /**
     * Create a script that sends coins directly to the given public key (eg in
    ...
    /**
     * Creates a script that sends coins directly to the given public key. Same
     * as {@link Script#createOutputScript(byte[])} but more type safe.
     */
	...
	try {
	    ...
	    // reassemble itself into bytes.
	    ByteArrayOutputStream bits = new ByteArrayOutputStream(
		    signature.length + pubkey.length + 2);
    ...
    // //////////////////// Interface used during verification of
    // transactions/blocks ////////////////////////////////
	...
	if (opcode == OP_0)
	    return 0;
	if (opcode < OP_1 || opcode > OP_16) // This should absolutely never
					     // happen
	...
	for (int i = script.chunks.size() - 1; i >= 0; i--)
		...
		subScript.parse(script.chunks.get(i).data, 0,
			script.chunks.get(i).data.length);
    ...
    /**
     * <p>
     * Whether or not this is a scriptPubKey representing a pay-to-script-hash
     * output. In such outputs, the logic that controls reclamation is not
     * actually in the output at all. Instead there's just a hash, and it's up
     * to the spending input to provide a program matching that hash. This rule
     * is "soft enforced" by the network as it does not exist in Satoshis
     * original implementation. It means blocks containing P2SH transactions
     * that don't match correctly are considered valid, but won't be mined upon,
     * so they'll be rapidly re-orgd out of the chain. This logic is defined by
     * <a href="https://en.feathercoin.it/wiki/BIP_0016">BIP 16</a>.
     * </p>
     *
     * <p>
     * feathercoinj does not support creation of P2SH transactions today. The
     * goal of P2SH is to allow short addresses even for complex scripts (eg,
     * multi-sig outputs) so they are convenient to work with in things like
     * QRcodes or with copy/paste, and also to minimize the size of the unspent
     * output set (which improves performance of the Feathercoin system).
     * </p>
     */
    public boolean isPayToScriptHash() {
	return program.length == 23 && (program[0] & 0xff) == OP_HASH160
		&& (program[1] & 0xff) == 0x14
	...
	while (cursor < inputScript.length) {
		...
		try {
		    bos.write(opcode);
		    bos.write(Arrays.copyOfRange(inputScript, cursor, cursor
			    + additionalBytes));
		} catch (IOException e) {
		    throw new RuntimeException(e);
		}
	...
	for (int i = 0; i < data.length; i++) {
	    // "Can be negative zero" -reference client (see OpenSSL's
	    // BN_bn2mpi)
	    if (data[i] != 0)
	...
	for (ScriptChunk chunk : script.chunks) {
		...
		if (opcode == OP_VERIF || opcode == OP_VERNOTIF)
		    throw new ScriptException(
			    "Script included OP_VERIF or OP_VERNOTIF");
		...
		if (opcode == OP_CAT || opcode == OP_SUBSTR
			...
			|| opcode == OP_RSHIFT)
		    throw new ScriptException(
			    "Script included a disabled Script Op.");
		...
		case OP_IF:
		    ...
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_IF on an empty stack");
		...
		case OP_NOTIF:
		    ...
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_NOTIF on an empty stack");
		    ifStack.add(!castToBool(stack.pollLast()));
		    continue;
		case OP_ELSE:
		    if (ifStack.isEmpty())
		...
		switch (opcode) {
		// case OP_0: dont know why this isnt also here in the reference
		// client
		case OP_1NEGATE:
		    stack.add(Utils.reverseBytes(Utils.encodeMPI(
			    BigInteger.ONE.negate(), false)));
		    break;
		case OP_1:
		case OP_2:
		case OP_3:
		...
		case OP_16:
		    stack.add(Utils.reverseBytes(Utils.encodeMPI(
			    BigInteger.valueOf(getOpNValue(opcode)), false)));
		    break;
		case OP_NOP:
		    break;
		case OP_VERIFY:
		...
		case OP_TOALTSTACK:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_TOALTSTACK on an empty stack");
		    altstack.add(stack.pollLast());
		    break;
		case OP_FROMALTSTACK:
		    if (altstack.size() < 1)
			throw new ScriptException(
				"Attempted OP_TOALTSTACK on an empty altstack");
		    stack.add(altstack.pollLast());
		    break;
		case OP_2DROP:
		    if (stack.size() < 2)
			throw new ScriptException(
				"Attempted OP_2DROP on a stack with size < 2");
		    stack.pollLast();
		    stack.pollLast();
		    break;
		case OP_2DUP:
		    if (stack.size() < 2)
			throw new ScriptException(
				"Attempted OP_2DUP on a stack with size < 2");
		...
		case OP_3DUP:
		    if (stack.size() < 3)
			throw new ScriptException(
				"Attempted OP_3DUP on a stack with size < 3");
		...
		case OP_2OVER:
		    if (stack.size() < 4)
			throw new ScriptException(
				"Attempted OP_2OVER on a stack with size < 4");
		...
		case OP_2ROT:
		    if (stack.size() < 6)
			throw new ScriptException(
				"Attempted OP_2ROT on a stack with size < 6");
		...
		case OP_2SWAP:
		    if (stack.size() < 4)
			throw new ScriptException(
				"Attempted OP_2SWAP on a stack with size < 4");
		...
		case OP_IFDUP:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_IFDUP on an empty stack");
		    if (castToBool(stack.getLast()))
			stack.add(stack.getLast());
		    break;
		case OP_DEPTH:
		    stack.add(Utils.reverseBytes(Utils.encodeMPI(
			    BigInteger.valueOf(stack.size()), false)));
		    break;
		case OP_DROP:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_DROP on an empty stack");
		    stack.pollLast();
		    break;
		case OP_DUP:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_DUP on an empty stack");
		    stack.add(stack.getLast());
		    break;
		case OP_NIP:
		    if (stack.size() < 2)
			throw new ScriptException(
				"Attempted OP_NIP on a stack with size < 2");
		...
		case OP_OVER:
		    if (stack.size() < 2)
			throw new ScriptException(
				"Attempted OP_OVER on a stack with size < 2");
		...
		case OP_ROT:
		    if (stack.size() < 3)
			throw new ScriptException(
				"Attempted OP_ROT on a stack with size < 3");
		...
		case OP_TUCK:
		    if (stack.size() < 2)
			throw new ScriptException(
				"Attempted OP_SWAP on a stack with size < 2");
		...
		case OP_RIGHT:
		    throw new ScriptException(
			    "Attempted to use disabled Script Op.");
		case OP_SIZE:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_SIZE on an empty stack");
		    stack.add(Utils.reverseBytes(Utils.encodeMPI(
			    BigInteger.valueOf(stack.getLast().length), false)));
		    break;
		case OP_INVERT:
		case OP_AND:
		case OP_OR:
		...
		case OP_MAX:
		    ...
		    BigInteger numericOPnum2 = castToBigInteger(stack
			    .pollLast());
		    BigInteger numericOPnum1 = castToBigInteger(stack
			    .pollLast());
			...
			throw new RuntimeException(
				"Opcode switched at runtime?");
		    }

		    stack.add(Utils.reverseBytes(Utils.encodeMPI(
			    numericOPresult, false)));
		    break;
		case OP_MUL:
		case OP_DIV:
		case OP_MOD:
		...
		case OP_NUMEQUALVERIFY:
		    ...
		    BigInteger OPNUMEQUALVERIFYnum2 = castToBigInteger(stack
			    .pollLast());
		    BigInteger OPNUMEQUALVERIFYnum1 = castToBigInteger(stack
			    .pollLast());
		...
		case OP_RIPEMD160:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_RIPEMD160 on an empty stack");
		...
		case OP_SHA1:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_SHA1 on an empty stack");
		    try {
			stack.add(MessageDigest.getInstance("SHA-1").digest(
				stack.pollLast()));
		...
		case OP_SHA256:
		    if (stack.size() < 1)
			throw new ScriptException(
				"Attempted OP_SHA256 on an empty stack");
		    try {
			stack.add(MessageDigest.getInstance("SHA-256").digest(
				stack.pollLast()));
		    } catch (NoSuchAlgorithmException e) {
			throw new RuntimeException(e); // Cannot happen.
		    }
		...
		case OP_CHECKSIGVERIFY:
		    ...
		    byte[] CHECKSIGconnectedScript = Arrays.copyOfRange(
			    script.program, lastCodeSepLocation,
			    script.program.length);

		    ByteArrayOutputStream OPCHECKSIGOutStream = new ByteArrayOutputStream(
			    CHECKSIGsig.length + 1);
		    try {
			writeBytes(OPCHECKSIGOutStream, CHECKSIGsig);
		    } catch (IOException e) {
			throw new RuntimeException(e); // Cannot happen
		    }
		    CHECKSIGconnectedScript = removeAllInstancesOf(
			    CHECKSIGconnectedScript,
			    OPCHECKSIGOutStream.toByteArray());

		    // TODO: Use int for indexes everywhere, we can't have that
		    // many inputs/outputs
		    Sha256Hash CHECKSIGhash = txContainingThis
			    .hashTransactionForSignature((int) index,
				    CHECKSIGconnectedScript,
				    CHECKSIGsig[CHECKSIGsig.length - 1]);

		    boolean CHECKSIGsigValid;
		    try {
			CHECKSIGsigValid = ECKey.verify(
				CHECKSIGhash.getBytes(), Arrays.copyOf(
					CHECKSIGsig, CHECKSIGsig.length - 1),
				CHECKSIGpubKey);
		    } catch (Exception e1) {
			// There is (at least) one exception that could be hit
			// here (EOFException, if the sig is too short)
			// Because I can't verify there aren't more, we use a
			// very generic Exception catch
			CHECKSIGsigValid = false;
		    }

		    if (opcode == OP_CHECKSIG)
			stack.add(CHECKSIGsigValid ? new byte[] { 1 }
				: new byte[] { 0 });
		...
		case OP_CHECKMULTISIGVERIFY:
		    ...
		    int CHECKMULTISIGpubKeyCount = castToBigInteger(
			    stack.pollLast()).intValue();
		    if (CHECKMULTISIGpubKeyCount < 0
			    || CHECKMULTISIGpubKeyCount > 20)
			throw new ScriptException(
		    ...
		    int CHECKMULTISIGsigCount = castToBigInteger(
			    stack.pollLast()).intValue();
		    if (CHECKMULTISIGsigCount < 0
			    || CHECKMULTISIGsigCount > CHECKMULTISIGpubKeyCount)
			throw new ScriptException(
		    ...
		    byte[] CHECKMULTISIGconnectedScript = Arrays.copyOfRange(
			    script.program, lastCodeSepLocation,
			    script.program.length);

		    for (byte[] CHECKMULTISIGsig : CHECKMULTISIGsigs) {
			ByteArrayOutputStream OPCHECKMULTISIGOutStream = new ByteArrayOutputStream(
				CHECKMULTISIGsig.length + 1);
			try {
			    writeBytes(OPCHECKMULTISIGOutStream,
				    CHECKMULTISIGsig);
			} catch (IOException e) {
			    throw new RuntimeException(e); // Cannot happen
			}
			CHECKMULTISIGconnectedScript = removeAllInstancesOf(
				CHECKMULTISIGconnectedScript,
				OPCHECKMULTISIGOutStream.toByteArray());
		    }

		    boolean CHECKMULTISIGValid = true;
		    while (CHECKMULTISIGsigs.size() > 0) {
			byte[] CHECKMULTISIGsig = CHECKMULTISIGsigs.getFirst();
			byte[] CHECKMULTISIGpubKey = CHECKMULTISIGpubkeys
				.pollFirst();

			// We could reasonably move this out of the loop,
			// but because signature verification is significantly
			// more expensive than hashing, its not a big deal
			Sha256Hash CHECKMULTISIGhash = txContainingThis
				.hashTransactionForSignature(
					(int) index,
					CHECKMULTISIGconnectedScript,
					CHECKMULTISIGsig[CHECKMULTISIGsig.length - 1]);
			try {
			    if (ECKey.verify(CHECKMULTISIGhash.getBytes(),
				    Arrays.copyOf(CHECKMULTISIGsig,
					    CHECKMULTISIGsig.length - 1),
				    CHECKMULTISIGpubKey))
				CHECKMULTISIGsigs.pollFirst();
			} catch (Exception e) {
			    // There is (at least) one exception that could be
			    // hit here (EOFException, if the sig is too short)
			    // Because I can't verify there aren't more, we use
			    // a very generic Exception catch
			}

			if (CHECKMULTISIGsigs.size() > CHECKMULTISIGpubkeys
				.size()) {
		    ...
		    // We uselessly remove a stack object to emulate a reference
		    // client bug
		    stack.pollLast();

		    if (opcode == OP_CHECKMULTISIG)
			stack.add(CHECKMULTISIGValid ? new byte[] { 1 }
				: new byte[] { 0 });
     ...
     *            The index in txContainingThis of the scriptSig (note: NOT the
     *            index of the scriptPubKey).
     * @param scriptPubKey
     *            The connected scriptPubKey containing the conditions needed to
     *            claim the value.
     * @param enforceP2SH
     *            Whether "pay to script hash" rules should be enforced. If in
     *            doubt, set to true.
     * @throws VerificationException
     *             if this script does not correctly spend the scriptPubKey
     */
    public void correctlySpends(Transaction txContainingThis,
	    long scriptSigIndex, Script scriptPubKey, boolean enforceP2SH)
	    throws ScriptException {
	...
	// P2SH is pay to script hash. It means that the scriptPubKey has a
	// special form which is a valid
	// program but it has "useless" form that if evaluated as a normal
	// program always returns true.
	// Instead, miners recognize it as special based on its template - it
	// provides a hash of the real scriptPubKey
	// and that must be provided by the input. The goal of this bizarre
	// arrangement is twofold:
	//
	// (1) You can sum up a large, complex script (like a CHECKMULTISIG
	// script) with an address that's the same
	// size as a regular address. This means it doesn't overload scannable
	// QR codes/NFC tags or become
	// un-wieldy to copy/paste.
	// (2) It allows the working set to be smaller: nodes perform best when
	// they can store as many unspent outputs
	// in RAM as possible, so if the outputs are made smaller and the inputs
	// get bigger, then it's better for
	// overall scalability and performance.

	// TODO: Check if we can take out enforceP2SH if there's a checkpoint at
	...
	if (enforceP2SH && scriptPubKey.isPayToScriptHash()) {
	    ...
	    Script scriptPubKeyP2SH = new Script(params, scriptPubKeyBytes, 0,
		    scriptPubKeyBytes.length);

	    executeScript(txContainingThis, scriptSigIndex, scriptPubKeyP2SH,
		    p2shStack);
